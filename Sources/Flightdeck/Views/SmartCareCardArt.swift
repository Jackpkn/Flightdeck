    ...
    var body: some View {
        Canvas { context, size in
            ...
            for i in 0..<particleCount {
                ...
                let px = ringRx * cos(pAngle)
                let py = ringRy * sin(pAngle)
                let rotatedPx = px * cos(ringAngle.radians) - py * sin(ringAngle.radians)
                let rotatedPy = px * sin(ringAngle.radians) + py * cos(ringAngle.radians)
    ...
    var body: some View {
        Canvas { context, size in
            ...
            for i in 0..<4 {
                let a = Double(i) * (.pi / 2)
                var tick = Path()
                tick.move(to: CGPoint(x: center.x + (rSize - 4) * cos(a), y: center.y + (rSize - 4) * sin(a)))
                tick.addLine(to: CGPoint(x: center.x + (rSize + 6) * cos(a), y: center.y + (rSize + 6) * sin(a)))
