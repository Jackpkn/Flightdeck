...
enum PricingTable {
    ...
    static let rates: [String: Rates] = [
        "claude-fable-5":            Rates(input: 8,   output: 40, cacheRead: 0.8,  cacheWrite: 10.0),
        "claude-fable-5-1":          Rates(input: 8,   output: 40, cacheRead: 0.8,  cacheWrite: 10.0),
        "claude-fable":              Rates(input: 8,   output: 40, cacheRead: 0.8,  cacheWrite: 10.0),
        "claude-mythos":             Rates(input: 15,  output: 75, cacheRead: 1.5,  cacheWrite: 18.75),
    ...
    static func cost(model: String?, usage: ClaudeLogLine.Usage?) -> Double {
        ...
        let r: Rates
        if let exact = rates[m] {
            r = exact
        } else if m.contains("fable") {
            r = rates["claude-fable-5"] ?? fallback
        } else if m.contains("mythos") {
            r = rates["claude-mythos"] ?? fallback
        } else if m.contains("opus") {
            r = rates["claude-opus-5"] ?? fallback
        } else if m.contains("sonnet") {
            r = rates["claude-sonnet-5"] ?? fallback
        } else if m.contains("haiku") {
            r = rates["claude-haiku-4-5-20251001"] ?? fallback
        } else {
            r = fallback
        }
