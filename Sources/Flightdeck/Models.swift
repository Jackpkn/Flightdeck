...
struct SessionAgg: Identifiable {
    ...
    /// Context window compaction threshold (80%).
    /// Beyond this threshold, Claude Code runs auto-compaction which lossily summarizes conversation history.
    static let compactionThreshold: Double = 0.80

    /// Whether this session is near context compaction (>= 80% of window used).
    /// Guarded by `contextWindowSource != .fallback` so attribution honesty is maintained.
    var isNearCompaction: Bool {
        contextWindowSource != .fallback && contextFraction >= Self.compactionThreshold
    }

    /// Generates a structured Task Handoff Prompt in GitHub-flavored Markdown.
    /// Used to migrate active task state to a fresh Claude Code / agent session
    /// before lossy auto-compaction degrades instruction memory and tool accuracy.
    var taskHandoffPrompt: String {
        var lines: [String] = []
        lines.append("# TASK HANDOFF: \(project.uppercased())")
        if !aiTitle.isEmpty {
            lines.append("> **Objective**: \(aiTitle)")
        }
        lines.append("")
        lines.append("## Previous Session Telemetry")
        ...
        let percent = Int(contextFraction * 100)
        let tokenStr = "\(Formatters.tokens(contextTokens)) / \(Formatters.tokens(contextTotalTokens)) (\(percent)%)"
        let pressureNote = isNearCompaction ? " ⚠️ COMPACTION IMMINENT" : ""
        lines.append("- **Context Utilization**: \(tokenStr)\(pressureNote)")
        lines.append("- **Session Spend**: \(Formatters.usd(totalCost)) (\(Formatters.tokens(totalTokens)) billed tokens)")
        lines.append("- **Model**: `\(displayModel)`")
        lines.append("")

        lines.append("## Files Modified in Previous Session")
        if filesModified.isEmpty {
            if !lastFile.isEmpty {
                lines.append("- `\(lastFile)` (active file)")
            } else {
                lines.append("- *(No file modifications recorded in session transcript)*")
            }
        } else {
            for file in filesModified.sorted() {
                lines.append("- `\(file)`")
            }
        }
        lines.append("")
        ...
        if !lastPrompt.isEmpty {
            lines.append("## Last Recorded Prompt")
            lines.append("```")
            lines.append(lastPrompt.trimmingCharacters(in: .whitespacesAndNewlines))
            lines.append("```")
            lines.append("")
        }

        lines.append("## Continuation Directives")
        lines.append("1. Run `git status -s` and `git diff` to inspect current working tree and uncommitted changes.")
        lines.append("2. Review the modified files listed above to maintain architectural alignment.")
        lines.append("3. Execute project test suites to verify baseline functionality before introducing new edits.")
        lines.append("4. Proceed with task execution in small, atomic steps to keep context window footprint minimal.")
        lines.append("")
        lines.append("---")
        lines.append("*Generated by Flightdeck Cockpit Telemetry*")
